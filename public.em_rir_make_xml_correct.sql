--Function: public.em_rir_make_xml_correct(uuid, uuid)

--DROP FUNCTION public.em_rir_make_xml_correct(uuid, uuid);

CREATE OR REPLACE FUNCTION public.em_rir_make_xml_correct
(
  IN  v_cduser  uuid,
  IN  v_cddoc   uuid
)
...
$$
  ...
  plan1=plpy.prepare('SELECT \'1\' as N_ZAP, ' \
                ...
                'DOCNAPR.DATNAPR as OLD_D_NAPR, ' \
                'case when (sltypesoc.cdtypesoc_u=\'2\' and slsocgr.cdsocgr_fed=\'1\' ) ' \
                ' then pac_parent.famip ' \
                ' else pacient.famip end as OLD_FAM, ' \
                'case when (sltypesoc.cdtypesoc_u=\'2\' and slsocgr.cdsocgr_fed=\'1\' )  ' \
                ' then pac_parent.namep ' \
                ' else  pacient.namep end as OLD_IM,' \
                'case when (sltypesoc.cdtypesoc_u=\'2\' and slsocgr.cdsocgr_fed=\'1\' )  ' \
                ' then pac_parent.otchp ' \
                ' else  pacient.otchp end as OLD_OT, ' \
                'case when (sltypesoc.cdtypesoc_u=\'2\' and slsocgr.cdsocgr_fed=\'1\' )  ' \
                '  then pac_parent.drogd::text ' \
                '  else pacient.drogd::text end as OLD_DR,' \
                ' DOCNAPR.change_date::date as D_NAPR, ' \
...
ALTER FUNCTION public.em_rir_make_xml_correct(uuid, uuid)
  OWNER TO postgres;